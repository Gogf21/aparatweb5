...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def _clear_auth_cookies(self):
        self._set_cookies({'auth_token': ''}, max_age=0)
    ...
    def do_GET(self):
        ...
        elif parsed_path.path == '/static/styles.css':
    ...
    def handle_form_submission(self):
        ...
        if errors:
            self.serve_form_with_errors(errors, data)
        else:
            try:
    ...
    def serve_main_page(self):
        try:
            ...
            if user:
                auth_section = """
                <div class="auth-info">
                    <p>Вы вошли как пользователь #{id}</p>
            ...
            if errors:
                if 'auth_error' in errors:
                    html_content = html_content.replace(
                        '<h2>Вход</h2>',
                        f'<h2>Вход</h2>\n<div class="error-message">{errors["auth_error"]}</div>'
                    )
                ...
                for field in ['username', 'password']:
                    if field in errors:
                        ...
                        error_div = f'<div class="error-message">{errors[field]}</div>'
                        html_content = html_content.replace(
                            f'<label for="{field}">',
            ...
            if errors:
                if 'server_error' in errors:
                    html_content = html_content.replace(
                        ...
                        f'<h2>Редактирование данных</h2>\n<div class="server-error">{errors["server_error"]}</div>'
                    )
                ...
                for field, error in errors.items():
                    if field != 'server_error':
                        ...
                        error_div = f'<div class="error-message">{error}</div>'
                        html_content = html_content.replace(
                            f'<label for="{field}">',
                            f'{error_div}<label for="{field}">'
                        )
